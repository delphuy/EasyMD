...
async function handleUpload() {
  ...
  for (let i = 0; i < pathsToUpload.length; i++) {
    ...
    try {
      ...
      const msg = (err as Error).message || t('localImage.uploadFailed')
      uploadErrors.value[path] = msg
      toast.error(msg)
    }
  ...
  const failed = Object.keys(uploadErrors.value).length
  if (failed > 0) {
    toast.error(t('localImage.uploadFailedCount', { count: failed }))
  }
}
