...
import { ArrowUpDown, BookOpen, ChevronRight, ChevronsUpDown, Clock, Columns2, Ellipsis, Eye, FileText, Keyboard, ListTree, Monitor, Moon, PenLine, Pilcrow, Search, Smartphone, Sun, Type } from '@lucide/vue'
import {
  Popover,
  PopoverContent,
  ...
  >
    <TooltipProvider :delay-duration="300">
      ...
      <div class="ml-auto flex shrink-0 items-center gap-2.5 sm:gap-3.5">
        ...
        <!-- PC Web / Mobile 设备切换（预览可见时） -->
        <div
          v-if="!isMobile && showDeviceToggle"
          class="flex items-center gap-0.5 rounded-md border border-border/60 p-0.5"
        >
          <Tooltip>
            <TooltipTrigger as-child>
              <button
                :aria-label="t('footer.desktopPreview')"
                class="flex cursor-pointer items-center gap-0.5 rounded-sm px-1.5 py-0.5 transition-all duration-200"
            ...
            <TooltipContent side="top" :side-offset="6" class="text-xs text-muted-foreground">
              <p>{{ t('footer.desktopPreview') }}</p>
            </TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger as-child>
              <button
                :aria-label="t('footer.mobilePreview')"
                class="flex cursor-pointer items-center gap-0.5 rounded-sm px-1.5 py-0.5 transition-all duration-200"
