...
![示例图片](https://picsum.photos/seed/easymd/640/200)

轻松实现富媒体内容展示！

> 部分目标平台可能限制外链跳转，建议在正文中写明完整 URL。

> 也可使用「格式」菜单中的外链相关选项，将链接整理到文末引用区。

另外，使用 `<![alt](url),![alt](url)>` 语法可以创建横屏滑动幻灯片。建议使用相似尺寸的图片以获得最佳显示效果。
...
| 项目人员                                    | 邮箱                   | 微信号       |
| ------------------------------------------- | ---------------------- | ------------ |
| 示例用户 A | user-a@example.com | wx_demo_a |
| 示例用户 B | user-b@example.com | wx_demo_b |

这样的表格让数据展示更为清爽！

> 手动编写标记太麻烦？可用工具栏「表格」或菜单「插入 → 表格」快速插入。
