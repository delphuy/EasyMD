...
import { applyLocalImageSrcs } from '@/lib/preview/local-image-src'
...
export const useRenderStore = defineStore(`render`, () => {
  ...
  const render = (content: string, options?: { themeMode?: 'light' | 'dark' }) => {
    ...
    // 后处理 HTML
    let html = postProcessHtml(baseHtml, readingTimeResult, renderer)

    // Desktop: resolve relative/local images against the open document path
    try {
      const postStore = usePostStore()
      html = applyLocalImageSrcs(html, postStore.currentPost?.path ?? null)
    }
    catch {
      // post store may not be ready during early boot
    }
